...
fn main() {
    ...
    test_single_macro(&mut macro_expander, nested_pattern, "Nested ellipsis pattern");
    
    // Show what works - single level ellipsis
    let single_pattern = "(define-syntax test-single (syntax-rules () ((test-single (a ...) result) result)))";
    test_single_macro(&mut macro_expander, single_pattern, "Single ellipsis pattern");
    ...
    test_single_macro(&mut macro_expander, multi_lambda, "Multiple lambda patterns");
    
    if test_expansion(&mut macro_expander, "(cps-lambda (lambda () 42) identity)", "Zero-arg lambda") {
        test_expansion(&mut macro_expander, "(cps-lambda (lambda (x) x) identity)", "One-arg lambda");
        test_expansion(&mut macro_expander, "(cps-lambda (lambda (x y) x) identity)", "Two-arg lambda");
    }
    ...
    test_expansion(&mut macro_expander, "(do ((i 0)) ((> i 5) i) (display i))", "Simplified do");
    ...
    test_expansion(&mut macro_expander, "(cps-transform (lambda (x) x) identity)", "CPS lambda transform");
    test_expansion(&mut macro_expander, "(cps-transform (if #t 42 0) identity)", "CPS if transform");
    test_expansion(&mut macro_expander, "(cps-transform (begin 1 2 3) identity)", "CPS begin transform");
    
    println!("\n❌ What would fail with complex CPS:");
    println!("  (cps-transform (do ((i 0 (+ i 1))) ((> i 5) i) (display i)) identity)");
    println!("  This would require CPS transformation of do loops, which need nested ellipsis support");
    ...
    test_single_macro(&mut macro_expander, workaround_do, "Workaround: simplified do");
