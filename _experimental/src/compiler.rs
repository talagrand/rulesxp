...
impl Compiler {
    ...
    pub fn compile_value(&mut self, value: &Value, is_tail_position: bool) -> Result<(), CompileError> {
        match value {
            Value::Boolean(_) | Value::Integer(_) | Value::UInteger(_) | 
            Value::Real(_) | Value::String(_) => {
                // Literal values
                ...
                if let Some(first) = elements.first() {
                    if let Value::Symbol(operator) = first {
                        match operator.as_str() {
                            "quote" => {
                            if elements.len() != 2 {
                                return Err(CompileError("quote requires exactly one argument".to_string()));
                            }
                        ...
                        "if" => {
                            if elements.len() != 3 && elements.len() != 4 {
                                return Err(CompileError("if requires 2 or 3 arguments".to_string()));
                            }
                            ...
                            if elements.len() == 4 {
                                ...
                                if let Opcode::JumpIfFalse(ref mut offset) = &mut self.instructions[else_jump].opcode {
                                ...
                                if let Opcode::Jump(ref mut offset) = &mut self.instructions[end_jump].opcode {
                        ...
                        "lambda" => {
                            if elements.len() < 3 {
                                return Err(CompileError("lambda requires at least 2 arguments".to_string()));
                            }
                            ...
                            let params = match &elements[1] {
                                Value::List(param_list) => {
                                    ...
                                    for param in param_list {
                                        match param {
                                            ...
                                            _ => return Err(CompileError("Parameter must be a symbol".to_string())),
                                        }
                                    }
                                    (params, variadic)
                                }
                                ...
                                _ => return Err(CompileError("Parameter list must be a list or symbol".to_string())),
                            };
                            
                            // Create a new compiler for the lambda body
                            let mut lambda_compiler = Compiler::new(self.source_code.clone(), self.current_env.clone());
                            ...
                            let lambda_module = lambda_compiler.finish();
                        ...
                        "define" => {
                            if elements.len() != 3 {
                                return Err(CompileError("define requires exactly 2 arguments".to_string()));
                            }
                            ...
                            match &elements[1] {
                                ...
                                Value::List(func_def) => {
                                    ...
                                    if func_def.is_empty() {
                                        return Err(CompileError("Function definition cannot have empty parameter list".to_string()));
                                    }
                                    
                                    let func_name = match &func_def[0] {
                                        Value::Symbol(name) => name.clone(),
                                        _ => return Err(CompileError("Function name must be a symbol".to_string())),
                                    };
                                ...
                                _ => return Err(CompileError("define target must be symbol or function definition".to_string())),
                            }
                            ...
                            _ => {
                                ...
                                if arg_count > u8::MAX as usize {
                                    return Err(CompileError("Too many arguments".to_string()));
                                }
                    ...
                    return Err(CompileError("Invalid function call".to_string()));
                }
            }
            
            _ => {
                return Err(CompileError(format!("Cannot compile value: {:?}", value)));
            }
        }
    ...
    fn check_recursion(&mut self, function_name: &str, is_tail_position: bool) -> Result<(), CompileError> {
...
/// Compile a value to bytecode
pub fn compile(value: &Value, source_code: String, env: Rc<Environment>) -> Result<BytecodeModule, CompileError> {
