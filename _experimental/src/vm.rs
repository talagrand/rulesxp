            ...
            match &instruction.opcode {
                Opcode::LoadConst(index) => {
                    if *index >= module.constants.len() {
                        return Err(self.create_runtime_error("Invalid constant index".to_string(), module));
                    }
                ...
                Opcode::LoadVar(name_index) => {
                    if *name_index >= module.strings.len() {
                        return Err(self.create_runtime_error("Invalid string index".to_string(), module));
                    }
                    ...
                    match self.current_env.lookup(name) {
                        ...
                        None => return Err(self.create_runtime_error(format!("Undefined variable: {}", name), module)),
                    }
                }
                ...
                Opcode::Pop => {
                    if self.stack.is_empty() {
                        return Err(self.create_runtime_error("Stack underflow".to_string(), module));
                    }
                    self.stack.pop();
                }
                ...
                Opcode::StoreVar(name_index) => {
                    ...
                    if let Err(_) = Rc::try_unwrap(self.current_env.clone()) {
                        return Err(self.create_runtime_error("Cannot modify environment - implementation limitation".to_string(), module));
                    }
                    // For now, we'll skip mutation - this needs RefCell for proper implementation
                    return Err(self.create_runtime_error("Variable mutation not implemented yet".to_string(), module));
                }
                ...
                Opcode::Call(arg_count) => {
                    ...
                    if self.stack.len() < total_args {
                        return Err(self.create_runtime_error("Stack underflow in call".to_string(), module));
                    }
                    ...
                    match procedure {
                            ...
                            match arity {
                                crate::value::Arity::Exact(expected) => {
                                    if provided_args != expected {
                                        return Err(self.create_runtime_error(format!(
                        ...
                        Value::Procedure { params, body, env, variadic } => {
                ...
                Opcode::TailCall(arg_count) => {
                    ...
                    if self.stack.len() < total_args {
                        return Err(self.create_runtime_error("Stack underflow in tail call".to_string(), module));
                    }
                    // Get procedure from stack (it's at the top, most recent push)
                    let procedure = self.stack[self.stack.len() - 1].clone();
                ...
                Opcode::Jump(offset) => {
                    ...
                    if new_ip >= module.code.len() {
                        return Err(self.create_runtime_error("Jump out of bounds".to_string(), module));
                    }
                    self.ip = new_ip;
                ...
                Opcode::JumpIfFalse(offset) => {
                    if self.stack.is_empty() {
                        return Err(self.create_runtime_error("Stack underflow in conditional jump".to_string(), module));
                    }
                ...
                Opcode::Dup => {
                    if self.stack.is_empty() {
                        return Err(self.create_runtime_error("Stack underflow in dup".to_string(), module));
                    }
                ...
                Opcode::Cons => {
                    if self.stack.len() < 2 {
                        return Err(self.create_runtime_error("Stack underflow in cons".to_string(), module));
                    }
                    ...
                    match cdr {
                        ...
                        _ => {
                            ...
                            return Err(self.create_runtime_error("Cannot cons onto non-list (improper lists not supported)".to_string(), module));
                        }
                    }
                }
                ...
                Opcode::Car => {
                    if self.stack.is_empty() {
                        return Err(self.create_runtime_error("Stack underflow in car".to_string(), module));
                    }
                    let value = self.stack.pop().unwrap();
                    ...
                    match value {
                        Value::List(elements) => {
                            if elements.is_empty() {
                                return Err(self.create_runtime_error("Cannot take car of empty list".to_string(), module));
                            }
                            self.stack.push(elements[0].clone());
                        }
                        _ => {
                            return Err(self.create_runtime_error(format!("Cannot take car of {}", value.type_name()), module));
                        }
                    }
                }
                ...
                Opcode::Cdr => {
                    if self.stack.is_empty() {
                        return Err(self.create_runtime_error("Stack underflow in cdr".to_string(), module));
                    }
                    let value = self.stack.pop().unwrap();
                    ...
                    match value {
                        Value::List(elements) => {
                            if elements.is_empty() {
                                return Err(self.create_runtime_error("Cannot take cdr of empty list".to_string(), module));
                            }
                            let cdr_elements = elements[1..].to_vec();
                            self.stack.push(Value::List(cdr_elements));
                        }
                        _ => {
                            return Err(self.create_runtime_error(format!("Cannot take cdr of {}", value.type_name()), module));
                        }
                    }
                }
                ...
                Opcode::MakeList(count) => {
                    ...
                    if self.stack.len() < count {
                        return Err(self.create_runtime_error("Stack underflow in make-list".to_string(), module));
                    }
                ...
                Opcode::MakeClosure(_code_offset, _free_var_count) => {
                    ...
                    return Err(self.create_runtime_error("Closure creation not implemented yet".to_string(), module));
                }
                
                Opcode::MakeVector(count) => {
                    let count = *count as usize;
                    if self.stack.len() < count {
                        return Err(self.create_runtime_error("Stack underflow in make-vector".to_string(), module));
                    }
                    
                    // For now, vectors are not implemented, so this will fail
                    return Err(self.create_runtime_error("Vectors not implemented yet".to_string(), module));
                }
            }
...
mod tests {
    ...
    fn test_vm_list_operations() {
        ...
        let mut module = BytecodeModule {
            code: vec![
                Instruction { opcode: Opcode::LoadConst(0), source_offset: 0 },
                Instruction { opcode: Opcode::Car, source_offset: 0 },
                Instruction { opcode: Opcode::Return, source_offset: 0 },
            ],
            constants: vec![Value::List(vec![Value::Integer(1), Value::Integer(2), Value::Integer(3)])],
            strings: vec![],
        ...
        module.code = vec![
            Instruction { opcode: Opcode::LoadConst(0), source_offset: 0 },
            Instruction { opcode: Opcode::Cdr, source_offset: 0 },
            Instruction { opcode: Opcode::Return, source_offset: 0 },
        ];
    ...
    fn test_vm_cons_operation() {
        ...
        let module = BytecodeModule {
            code: vec![
                Instruction { opcode: Opcode::LoadConst(0), source_offset: 0 }, // Push 1
                Instruction { opcode: Opcode::LoadConst(1), source_offset: 0 }, // Push (2 3)
                Instruction { opcode: Opcode::Cons, source_offset: 0 },         // Cons them
                Instruction { opcode: Opcode::Return, source_offset: 0 },
            ],
            constants: vec![
                Value::Integer(1),
    ...
    fn test_vm_make_list() {
        ...
        let module = BytecodeModule {
            code: vec![
                Instruction { opcode: Opcode::LoadConst(0), source_offset: 0 }, // Push 1
                Instruction { opcode: Opcode::LoadConst(1), source_offset: 0 }, // Push 2  
                Instruction { opcode: Opcode::LoadConst(2), source_offset: 0 }, // Push 3
                Instruction { opcode: Opcode::MakeList(3), source_offset: 0 },  // Make list of 3 elements
                Instruction { opcode: Opcode::Return, source_offset: 0 },
            ],
            constants: vec![
                Value::Integer(1),
    ...
    fn test_vm_stack_operations() {
        ...
        let module = BytecodeModule {
            code: vec![
                Instruction { opcode: Opcode::LoadConst(0), source_offset: 0 }, // Push 42
                Instruction { opcode: Opcode::Dup, source_offset: 0 },          // Duplicate it
                Instruction { opcode: Opcode::Pop, source_offset: 0 },          // Pop one copy
                Instruction { opcode: Opcode::Return, source_offset: 0 },       // Return the remaining
            ],
    ...
    fn test_vm_builtin_call() {
        ...
        let module = BytecodeModule {
            code: vec![
                Instruction { opcode: Opcode::LoadConst(0), source_offset: 0 },  // Push 1
                Instruction { opcode: Opcode::LoadConst(1), source_offset: 0 },  // Push 2
                Instruction { opcode: Opcode::LoadVar(0), source_offset: 0 },    // Push + function
                Instruction { opcode: Opcode::Call(2), source_offset: 0 },       // Call with 2 args
                Instruction { opcode: Opcode::Return, source_offset: 0 },
            ],
            constants: vec![
                Value::Integer(1),
    ...
    fn test_vm_list_builtin_calls() {
        ...
        let module = BytecodeModule {
            code: vec![
                Instruction { opcode: Opcode::LoadConst(0), source_offset: 0 },  // Push (1 2 3)
                Instruction { opcode: Opcode::LoadVar(0), source_offset: 0 },    // Push car function
                Instruction { opcode: Opcode::Call(1), source_offset: 0 },       // Call with 1 arg
                Instruction { opcode: Opcode::Return, source_offset: 0 },
            ],
            constants: vec![
    ...
    fn test_basic_closure_functionality() {
        ...
        // Define make-adder
        vm.execute(&compile(&parse("(define (make-adder n) (lambda (x) (+ x n)))").unwrap(), 
                           "(define (make-adder n) (lambda (x) (+ x n)))".to_string(), 
                           vm.current_env.clone()).unwrap()).unwrap();
        
        // Create add-5 closure
        vm.execute(&compile(&parse("(define add-5 (make-adder 5))").unwrap(),
                           "(define add-5 (make-adder 5))".to_string(), 
                           vm.current_env.clone()).unwrap()).unwrap();
        
        // Test the closure
        let result = vm.execute(&compile(&parse("(add-5 10)").unwrap(),
                                        "(add-5 10)".to_string(), 
                                        vm.current_env.clone()).unwrap()).unwrap();
        assert_eq!(result, Value::Integer(15));
    }
    ...
    fn test_multiple_independent_closures() {
        ...
        vm.execute(&compile(&parse("(define (make-multiplier n) (lambda (x) (* x n)))").unwrap(),
                           "(define (make-multiplier n) (lambda (x) (* x n)))".to_string(), 
                           vm.current_env.clone()).unwrap()).unwrap();
        
        // Create two different multipliers
        vm.execute(&compile(&parse("(define double (make-multiplier 2))").unwrap(),
                           "(define double (make-multiplier 2))".to_string(), 
                           vm.current_env.clone()).unwrap()).unwrap();
        
        vm.execute(&compile(&parse("(define triple (make-multiplier 3))").unwrap(),
                           "(define triple (make-multiplier 3))".to_string(), 
                           vm.current_env.clone()).unwrap()).unwrap();
        
        // Test both closures maintain separate state
        let result1 = vm.execute(&compile(&parse("(double 5)").unwrap(),
                                         "(double 5)".to_string(), 
                                         vm.current_env.clone()).unwrap()).unwrap();
        assert_eq!(result1, Value::Integer(10));
        
        let result2 = vm.execute(&compile(&parse("(triple 5)").unwrap(),
                                         "(triple 5)".to_string(), 
                                         vm.current_env.clone()).unwrap()).unwrap();
    ...
    fn test_nested_closure_creation() {
        ...
        vm.execute(&compile(&parse("(define make-double-scaler (make-scaler-factory 2))").unwrap(),
                           "(define make-double-scaler (make-scaler-factory 2))".to_string(), 
                           vm.current_env.clone()).unwrap()).unwrap();
        
        // Create a specific scaler
        vm.execute(&compile(&parse("(define scale-by-3 (make-double-scaler 3))").unwrap(),
                           "(define scale-by-3 (make-double-scaler 3))".to_string(), 
                           vm.current_env.clone()).unwrap()).unwrap();
        
        // Test the nested closure
        let result = vm.execute(&compile(&parse("(scale-by-3 4)").unwrap(),
    ...
    fn test_closure_composition() {
        ...
        vm.execute(&compile(&parse("(define (compose f g) (lambda (x) (f (g x))))").unwrap(),
                           "(define (compose f g) (lambda (x) (f (g x))))".to_string(), 
                           vm.current_env.clone()).unwrap()).unwrap();
        
        // Create individual adders
        vm.execute(&compile(&parse("(define add-1 (make-adder 1))").unwrap(),
                           "(define add-1 (make-adder 1))".to_string(), 
                           vm.current_env.clone()).unwrap()).unwrap();
        
        vm.execute(&compile(&parse("(define add-2 (make-adder 2))").unwrap(),
                           "(define add-2 (make-adder 2))".to_string(), 
                           vm.current_env.clone()).unwrap()).unwrap();
        
        // Compose them
        vm.execute(&compile(&parse("(define add-3-total (compose add-1 add-2))").unwrap(),
                           "(define add-3-total (compose add-1 add-2))".to_string(), 
                           vm.current_env.clone()).unwrap()).unwrap();
        
        // Test composition
        let result = vm.execute(&compile(&parse("(add-3-total 10)").unwrap(),
    ...
    fn test_multi_variable_capture() {
        ...
        vm.execute(&compile(&parse("(define global-multiplier 100)").unwrap(),
        ...
        vm.execute(&compile(&parse("(define calc (make-calculator 15))").unwrap(),
                           "(define calc (make-calculator 15))".to_string(), 
                           vm.current_env.clone()).unwrap()).unwrap();
        
        // Test multi-variable capture
        let result = vm.execute(&compile(&parse("(calc 0)").unwrap(),
                                        "(calc 0)".to_string(), 
                                        vm.current_env.clone()).unwrap()).unwrap();
    ...
    fn test_environment_isolation() {
        ...
        vm.execute(&compile(&parse("(define (make-counter start) (lambda (increment) (+ start increment)))").unwrap(),
                           "(define (make-counter start) (lambda (increment) (+ start increment)))".to_string(), 
                           vm.current_env.clone()).unwrap()).unwrap();
        
        // Create two counters with different starting values
        vm.execute(&compile(&parse("(define counter1 (make-counter 10))").unwrap(),
                           "(define counter1 (make-counter 10))".to_string(), 
                           vm.current_env.clone()).unwrap()).unwrap();
        
        vm.execute(&compile(&parse("(define counter2 (make-counter 20))").unwrap(),
                           "(define counter2 (make-counter 20))".to_string(), 
                           vm.current_env.clone()).unwrap()).unwrap();
        
        // Test that they maintain separate environments
        let result1 = vm.execute(&compile(&parse("(counter1 5)").unwrap(),
                                         "(counter1 5)".to_string(), 
                                         vm.current_env.clone()).unwrap()).unwrap();
        assert_eq!(result1, Value::Integer(15)); // 10 + 5
        
        let result2 = vm.execute(&compile(&parse("(counter2 5)").unwrap(),
